...
String TricorderConfig::generateUniqueId() {
  ...
  char uniqueId[16];
  snprintf(uniqueId, sizeof(uniqueId), "IVST%04X", uniquePart);
...
void TricorderConfig::setDefaults() {
  ...
  // Device settings - Updated for IV Station branding
  strcpy(config.deviceLabel, ("IV-Station-" + uniqueId.substring(4)).c_str()); // Use last part after "IVST"
  strcpy(config.propId, uniqueId.c_str());
  strcpy(config.description, "IV Station Medical Prop");
  ...
  strcpy(config.hostname, ("ivstation-" + hostnameSuffix).c_str());
