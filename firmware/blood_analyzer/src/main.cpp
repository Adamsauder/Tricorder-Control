/*
 * Blood Analyzer Firmware - sACN Compatible RGBW LED Device
 * ESP32-C3 XIAO based Blood Analyzer with 7 WS2812B RGBW LED control and web configuration
 */
...
#define LED_PIN D3    // D3 (GPIO5) - 7 WS2812B RGBW LEDs
#define NUM_LEDS 7    // 7 RGBW LEDs
...
String deviceLabel = "Blood Analyzer 001";
String deviceType = "blood_analyzer";
String firmwareVersion = "Blood Analyzer v1.0 SACN DMX421";
int sacnUniverse = 1;
int sacnStartAddress = 421;  // Default DMX address 421 for Blood Analyzers
...
String generateUniqueDeviceId() {
  ...
  char uniqueId[16];
  snprintf(uniqueId, sizeof(uniqueId), "BA%04X", uniquePart);
...
void setup() {
  ...
  Serial.println("Blood Analyzer Control System Starting...");
  Serial.println("Hardware: ESP32-C3 XIAO with 7 WS2812B RGBW LEDs");
  ...
  // Initialize 7 WS2812B RGBW LEDs
  ...
  Serial.println("Blood Analyzer initialization complete");
  Serial.println("Ready for sACN and UDP commands");
  
  // Idle state - dim blue for blood analyzer
  setLEDColor(0, 0, 64, 0);  // Dim blue for idle state
  lastActivity = millis();
}
...
void loop() {
  ...
  if (deviceActive && (millis() - lastActivity > 30000)) {
    ...
    deviceActive = false;
    setLEDColor(0, 0, 64, 0);  // Dim blue for idle
  }
...
void loadConfiguration() {
  ...
  if (propConfig.loadConfig(config)) {
    ...
    if (!config.deviceLabel.isEmpty() && !config.deviceLabel.startsWith("BLOOD_ANALYZER_")) {
...
void initializeWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (MDNS.begin(hostname.c_str())) {
      ...
      MDNS.addService("blood-analyzer", "udp", UDP_PORT);
    }
...
<html>
<head>
    <title>Blood Analyzer Control</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <style>
        body { font-family: Arial; margin: 20px; background: #1a1a2a; color: #fff; }
        .container { max-width: 600px; margin: 0 auto; background: #2a2a3a; padding: 20px; border-radius: 10px; }
        .header { text-align: center; color: #6b6bff; margin-bottom: 30px; }
        .status { background: #3a3a4a; padding: 15px; border-radius: 5px; margin: 10px 0; }
        ...
        .config { background: #3a3a4a; padding: 15px; border-radius: 5px; margin: 10px 0; }
        input, select { width: 100%; padding: 8px; margin: 5px 0; border: 1px solid #666; border-radius: 3px; background: #1a1a2a; color: #fff; }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1>🧪 Blood Analyzer Control</h1>
            <p>ESP32-C3 7 RGBW LED Controller</p>
        </div>
    ...
    <script>
        ...
        .then(response => response.json())
        .then(config => {
            document.getElementById('deviceLabel').value = config.deviceLabel || 'Blood_Analyzer_001';
            document.getElementById('sacnUniverse').value = config.sacnUniverse || 1;
            document.getElementById('dmxAddress').value = config.dmxStartAddress || 421;
  ...
  server.on("/test", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    setLEDColor(0, 0, 64, 0);  // Dim blue for Blood Analyzer
    
    request->send(200, "text/plain", "Blood Analyzer LED test sequence complete!");
  });
  ...
  server.begin();
  Serial.println("Blood Analyzer web server started on port 80");
}
void handleUDPCommands() {
  ...
  if (packetSize > 0) {
    ...
    if (error == DeserializationError::Ok) {
      ...
      if (target == deviceId || target == "ALL" || target.startsWith("BLOOD_ANALYZER")) {
...
void setLEDFromSACN() {
  if (!sacnActive || sacnStartAddress < 1 || sacnStartAddress > 485) {  // 7 LEDs * 4 channels = 28 channels, so max start is 485
    return;
  }
  
  // 7 RGBW LEDs use 28 channels total (4 channels per LED)
...
void setLEDColor(uint8_t r, uint8_t g, uint8_t b, uint8_t w) {
  ...
  if (sacnPriority && sacnActive) {
    Serial.println("  LED update blocked by sACN priority!");
    return;
  }
...
void setLEDPattern(String pattern) {
  if (pattern == "off") {
    ...
    setLEDColor(0, 0, 64, 0);   // Dim blue for idle (analyzer theme)
    deviceActive = false;
  }
}
