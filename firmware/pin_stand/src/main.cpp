...
String deviceLabel = "Pin Stand 001";
String deviceType = "iv_injector";
const char* FIRMWARE_VERSION = "Pin Stand v1.0";
...
String generateUniqueDeviceId() {
  ...
  char uniqueId[16];
  snprintf(uniqueId, sizeof(uniqueId), "PIN%04X", uniquePart);
...
void loadConfiguration() {
  ...
  if (propConfig.loadConfig(config)) {
    ...
    if (!config.deviceLabel.isEmpty() && !config.deviceLabel.startsWith("PIN_STAND_")) {
...
<body>
    <div class="container">
        ...
        <div class="config">
            ...
            <input type="text" id="deviceLabel" value="PIN_STAND_001">
    ...
    <script>
        ...
        .then(response => response.json())
        .then(config => {
            document.getElementById('deviceLabel').value = config.deviceLabel || 'PIN_STAND_001';
