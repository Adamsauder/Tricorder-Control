...
bool PropConfig::resetToDefaults() {
    ...
    defaultConfig.deviceLabel = "DEFRAGMENTOR_001";
    defaultConfig.sacnUniverse = 1;
    defaultConfig.dmxStartAddress = 1;
    defaultConfig.deviceType = "defragmentor";
    defaultConfig.numLeds = 50;
...
String PropConfig::getDeviceLabel() {
    ...
    String value = prefs.getString(KEY_DEVICE_LABEL, "DEFRAGMENTOR_001");
    prefs.end();
    return value;
}
...
String PropConfig::getDeviceType() {
    if (!prefs.begin(NAMESPACE, true)) return "defragmentor";
    String value = prefs.getString(KEY_DEVICE_TYPE, "defragmentor");
    prefs.end();
    return value;
}
...
int PropConfig::getNumLEDs() {
    if (!prefs.begin(NAMESPACE, true)) return 50;
    int value = prefs.getInt(KEY_NUM_LEDS, 50);
    prefs.end();
    return value;
}
...
int PropConfig::getFixtureNumber() {
    if (!prefs.begin(NAMESPACE, true)) return 3;
    int value = prefs.getInt(KEY_FIXTURE_NUMBER, 3);
    prefs.end();
    return value;
}
...
void PropConfig::printConfig() {
    ...
    Serial.println("=== Defragmentor Configuration ===");
